...
class inria(imdb):
    ...
    def _load_inria_annotation(self, index):
        ...
        #print "readin annotations: ", data

        with open(filename) as f:
            lines = f.readlines()
            lines = [lines[i].split(' ') for i in range(1, len(lines))] # first line is version, skip
            if len(lines) != 0:
                gt = np.array(lines)[:,1:5].astype(int)
                gt = np.hstack((gt, np.zeros((len(gt),1), int)))
            else:
                gt = None

        num_objs = len(gt)
        #print "Number of objects in this frame: ", num_objs
	    ...
	    coor = gt[ix, :]
            x1 = float(coor[0])
            y1 = float(coor[1])
            x2 = float(coor[0] + coor[2])
            y2 = float(coor[1] + coor[3])
            x1 = np.amax(x1, 0)
            y1 = np.amax(y1, 0)
            cls = self._class_to_ind['hand']
